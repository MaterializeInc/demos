...
The [docker-compose file](compose.yml) spins up containers with the following names, connections and roles:
...
1. It seeds Postgres with `item`, `user` and `purchase` tables, and then begins rapidly adding `purchase` rows that join an item and a user. _(~10 per second)_
2. It simultaneously begins sending JSON-encoded `pageview` events directly to kafka. _(~750 per second)_

As the database writes occur, Materialize subscribes to this change feed and maintains our materialized views with the incoming data––materialized views typically being some report whose information we're regularly interested in viewing.
...
1. Bring up the Docker Compose containers in the background:
   ...
   **This may take several minutes to complete the first time you run it.**
...
1. Create the Kafka and Schema Registry connections:

    ```sql
    -- Create Kafka connection
    CREATE CONNECTION redpanda TO KAFKA (
...
1. Now that you have your secrets and connections created, define all of the tables in the Postgres source database:
...
1. We'll also want to create a JSON-formatted source for the `pageviews`:
...
1. Next, we will create our first (non-materialized) view:
...
1. Let's create some more views:
