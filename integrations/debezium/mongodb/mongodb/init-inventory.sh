...
mongosh localhost:27017/inventory <<-EOF
    rs.initiate({
        _id: "rs0",
...
mongosh -u admin -p admin localhost:27017/admin --authenticationDatabase admin <<-EOF
    db.grantRolesToUser("admin", [{ role: "readWrite", db: "inventory" }]);
EOF

mongosh -u admin -p admin localhost:27017/inventory --authenticationDatabase admin <<-EOF
