...
sed -i "s/EXTERNAL_IP/${EXTERNAL_IP}/g" /deploy/register-postgres.json

curl -i -X POST -H "Accept:application/json" -H  "Content-Type:application/json" http://debezium:8083/connectors/ -d @/deploy/register-postgres.json
