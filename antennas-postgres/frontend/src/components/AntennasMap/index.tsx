 ...
 */
function setSourceData(map: MapBox, sourceName: string, data: Array<GeoJSON>): void {
...
function addPulsingDot(map: any, name: string, source: string, color: string, size?: number) {
  (map.current as any).addImage(name, buildPulsingDot(map.current as any, color, size || 30), {
    pixelRatio: 2,
  });
...
function handleTailEventUpdate(
  ...
  if (lastEvent) {
    const {timestamp: lastTimestamp, performance: lastPerformance} = lastEvent;

    if (diff > 0 && lastTimestamp < updateTimestamp) {
      antennasMap.set(antennaId, event);
    } else if (
      (lastPerformance === antennaPerformance && lastTimestamp <= updateTimestamp) ||
...
function createLayer(id: string, source: string, color: string): mapboxgl.AnyLayer {
  return {
    id,
    type: 'circle',
    source,
...
export default function AntennasMap() {
  ...
  const [antennasMap, setAntennasMap] = useState<Map<string, Antenna>>(new Map());
  const [antennasSupportedSet, setAntennasSupportedSet] = useState<Set<string>>(new Set());
  const markSetRef = useRef<Set<string>>(new Set());

  /**
   * GraphQL Subscription
   */
  const {error, data} = useSubscription<AntennasUpdatesSubscription>(SUBSCRIBE_ANTENNAS, {
  ...
  const onHelpersClick = useCallback(() => {
    ...
    if (mapBox) {
      mapBox.setLayoutProperty('helper-antennas-pulsing-dot', 'visibility', 'visible');

      mainLayers.forEach((layer) => mapBox.setLayoutProperty(layer, 'visibility', 'none'));
    }
  }, [mainLayers]);
  ...
  const onAntennaClick = useCallback(
    (event) => {
      ...
      if (mapBox) {
        ...
        // Disable helpers too
        mapBox.setLayoutProperty('helper-antennas-pulsing-dot', 'visibility', 'none');
      }
    },
    [mainLayers]
  );
  ...
  const onLoad = useCallback(() => {
    ...
    if (mapBox) {
      ...
      typesOfAntennas.forEach(({name, dotColor, layerColor}) => {
         ...
         */
        if (name !== 'helper') {
          mapBox.addLayer(createLayer(`${name}-antennas-layer`, `${name}-antennas`, layerColor));
        }

        /**
         * Add antenna pulsating dot
         */
        addPulsingDot(map, `${name}-antennas-pulsing-dot`, `${name}-antennas`, dotColor, 50);
      });
    }
  }, [typesOfAntennas]);
  ...
  return (
    <Box height={'100%'}>
      <Box display={'flex'} width={'100%'} height={'65%'} paddingX={'5rem'} overflow={'hidden'}>
        <UnorderedList
          minWidth={'400px'}
          maxWidth={'400px'}
          marginRight={'4rem'}
        ...
        >
          {Array.from(antennasMap.values())
            .sort((a, b) => (Number(a.antenna_id) > Number(b.antenna_id) ? 1 : -1))
              ...
              return (
                <ListItem key={x.antenna_id} marginBottom={'10px'}>
                  <Box display={'flex'}>
                    ...
                    {antennasSupportedSet.has(x.geojson.properties.name) && <span>🛠️</span>}
                  </Box>
                  <Box display={'flex'} fontSize={'md'}>
                    ...
                    >
                      <span style={{fontWeight: 300}}>📡 ID: {x.antenna_id}</span>
                      {'  -  '}
        ...
        <Box id="map" width={'100%'} boxShadow={'xl'} ref={mapContainer} className="map" />
      </Box>
